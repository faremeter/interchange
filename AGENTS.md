...
The hub exposes a self-documenting REST API. The route reference is
generated, not hand-written; see the "HTTP API" section of the README for
the full mechanism. The essentials for working in this repository:

- The running hub serves the live OpenAPI spec at `GET /openapi.json`.
- The human-readable reference lives in `docs/API.md`. It is generated by
  `bin/gen-api-docs.ts` (`make docs`) from ArkType introspection over the
  type definitions in `@intx/types`; `make lint` fails if it is stale. Do
  not edit `docs/API.md` by hand.
- `docs/ROUTES.md` explains the route design and conventions.

Read these sources before you infer the API shape from the route handlers.
