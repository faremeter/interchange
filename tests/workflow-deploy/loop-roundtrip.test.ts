...
describe.skipIf(!harnessDbEnvAvailable())(
  ...
  () => {
    ...
    test("the loop converges after three iterations and the run completes", async () => {
      ...
      // the workflow has no onTrigger or childWorkflow primitive. The child
      // packs the ChildSpawned records incrementally and replication can lag
      // under load, so read until the third record lands rather than racing
      // replication. The poll carries no deadline of its own: this test's own
      // budget is the failsafe for a count that never arrives, and the harness
      // `waitFor` is what puts the sidecar's output on the env teardown's
      // report.
      let loopSpawns = 0;
      await waitFor(async () => {
