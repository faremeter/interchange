// Agent-to-workflow fold synthesis.
//
...
// stored as. It is PURE: every impure input (the inference preferences
// resolved against the tenant catalog, the mail trigger address) is supplied
// by the caller, not computed here. That keeps the synthesis a deterministic
// function of its inputs and lets the backfill own the catalog and address
// resolution.
//
...
// an agent instance does). The step agent's `toolFactories` is the empty
// array by construction -- a folded definition has no live factories, and the
