// Regression coverage for H-D1: the trivial-wrap path must surface
...
// `wrapHarnessAsTrivialAgent` emitted `toolFactories: []` regardless
// of `HarnessConfig.tools`, and the walk emitted zero `tool:` grants.
// The gate then trivially admitted every deploy whose only path was
// the trivial branch, defeating the agent-deploy uniformity claim's
...
import { wrapHarnessAsTrivialAgent } from "./orchestrator";
...
describe("H-D1: trivial wrap surfaces HarnessConfig.tools to the walk", () => {
  test("walk emits a tool: grant per HarnessConfig.tools entry", () => {
    const trivialAgent: AgentDefinition<BaseEnv> = wrapHarnessAsTrivialAgent({
      config: HARNESS_CONFIG_WITH_TOOLS,
      deployContent: { systemPrompt: "legacy" },
    });
    expect(trivialAgent.toolFactories.length).toBe(2);
    const factoryIds = trivialAgent.toolFactories.map((f) => f.id);
  ...
  test("walk emits no tool: grants when HarnessConfig.tools is empty", () => {
    ...
    expect(trivialAgent.toolFactories).toEqual([]);
