...
This section describes the cross-provider content taxonomy the inference layer speaks. The day-one taxonomy ("Message Format › Content Types") covers text, thinking, image input, tool call, and tool result; that set is sufficient for Anthropic and OpenAI-compatible providers but does not accommodate Gemini's additional surfaces — audio, video, and PDF inputs; image output; thought signatures; Google Search grounding citations; the Files API; and request-shape knobs that have no portable analogue.

The rules below are grounded in a 2026-05-20 capture campaign against the Gemini Developer API v1beta surface and an extension of that campaign against the OpenCode Zen relay covering five OpenAI-compatible upstream models. Twelve Gemini capability slices are recorded under `packages/inference-testing/wire/gemini/`; thirty-three OpenAI-compatible capability slices are recorded under `packages/inference-testing/wire/opencode-zen/` (five models times text, function-calling, and reasoning capabilities — streaming and non-streaming — plus vision-input for the three vision-capable models). Each capture comprises the live request bytes, response bytes (or SSE stream), headers, and a `metadata.json` index. The per-capability observed-vs-documented analysis lives in `docs/GEMINI_DISCOVERY.md` and `docs/OPENCODE_DISCOVERY.md`. The captures are the ground truth — every rule below cites at least one capture path as evidence.

### Capability surface

The union of capabilities the taxonomy accommodates, drawn from the three day-one providers (Anthropic, OpenAI-compatible, Gemini), is wider than the day-one content-block set. Inputs include text, base64 inline images, base64 inline audio, base64 inline video, base64 inline PDF (and other documents), and file references obtained via a prior upload. Outputs include text, thinking with optional opaque signatures, tool calls, tool results, generated images, and grounded text with citation spans linking back to web sources. Request-shape inputs include tool definitions, thinking configuration, response modalities, server-side tool toggles (code execution, Google Search), and provider-specific knobs that have no portable analogue.

The Gemini capture corpus at `packages/inference-testing/wire/gemini/` is the concrete evidence base for the rules that follow: `text-non-streaming`, `text-streaming`, `function-calling-multi-turn`, `function-calling-thinking`, `image-input`, `image-output`, `audio-input`, `video-input`, `pdf-input`, `code-execution`, `google-search-grounding`, and `files-api`.

### Media sources

Per-modality content blocks (`image`, `audio`, `video`, `document`) share a discriminated `source` field naming `inline` (base64 bytes plus MIME type) or `file_ref` (provider-issued reference plus MIME type). The existing `image` block becomes the `inline` case of this shape; new per-modality blocks for audio, video, and document follow the same pattern. The `source` discriminant carries the upload-flow indirection without coupling the modality vocabulary to it.

The captured Gemini requests place modality and source on independent axes. Inline image (`packages/inference-testing/wire/gemini/image-input/request.json`), inline audio (`packages/inference-testing/wire/gemini/audio-input/request.json`), inline video (`packages/inference-testing/wire/gemini/video-input/request.json`), and inline PDF (`packages/inference-testing/wire/gemini/pdf-input/request.json`) all carry a part with `inlineData: {mimeType, data}`. A file-reference part (`packages/inference-testing/wire/gemini/files-api/generate/request.json`) uses `fileData: {mimeType, fileUri}` regardless of the underlying modality. The `promptTokensDetails` arrays in the captured responses label modalities explicitly (`TEXT`, `IMAGE`, `AUDIO`, `VIDEO`, `DOCUMENT`); `packages/inference-testing/wire/gemini/video-input/response.json` splits a single mp4 part into `VIDEO` and `AUDIO` cost lines, confirming that modality is a billing-relevant axis the taxonomy surfaces independently of the inline-versus-reference distinction.

On the OpenAI-compatible side, the captured OpenCode-Zen vision requests confirm the inline-image shape `{type: "image_url", image_url: {url: "data:image/jpeg;base64,..."}}` is the lowest-common-denominator across vision-capable vendors. The captures at `packages/inference-testing/wire/opencode-zen/kimi-k2.6/vision-input/request.json`, `packages/inference-testing/wire/opencode-zen/qwen3.6-plus/vision-input/request.json`, and `packages/inference-testing/wire/opencode-zen/mimo-v2-omni/vision-input/request.json` all use the object form of `image_url` with a `data:` URI and receive a non-empty `choices[0].message.content` describing the image. The response is always a single string, not a content-parts array: the relay flattens multimodal output back to plain text on the way out (`packages/inference-testing/wire/opencode-zen/kimi-k2.6/vision-input/response.json`). Image cost is reported on the OpenAI-compatible side as `usage.prompt_tokens_details.image_tokens` on the vendors that provide that breakdown. Two of the five OpenCode-Zen models reject vision input: `deepseek-v4-pro` returns HTTP 400 `invalid_request_error: "unknown variant 'image_url', expected 'text'"` when sent a multimodal content array, and `glm-5.1` returns HTTP 200 with a refusal text rather than a description. Both are recorded as `vision: false` in `bin/opencode-discover/models.ts`; the taxonomy's per-modality capability flags must accommodate this kind of relay-level not-supported signal.

### Image output and media spill

Image output is emitted as a single `inference.media.complete` event carrying the modality, MIME type, and bytes (or a context-store reference if a transform has spilled them). There is no delta accumulation; the streaming harness emits the event at the point the part is finalized, regardless of whether the underlying provider call is streamed or non-streamed.

The captured Gemini image-output response (`packages/inference-testing/wire/gemini/image-output/response.json`) is a single non-streaming `generateContent` call that returns one candidate with two parts: a text caption followed by an `inlineData` part with `mimeType: "image/png"` and a base64 payload approximately 1.17 MB on disk. The image is fully formed in one response; the provider does not chunk it. The streaming counterpart at `packages/inference-testing/wire/gemini/image-output-streaming/response.sse` confirms the same shape under `streamGenerateContent`: the full ~380 KB base64 PNG arrives in a single SSE event, not split across deltas. A multi-megabyte payload sitting inline in a content block bloats every subsequent prompt that replays the assistant turn, so the transforms pipeline handles spill. A media-spill transform — structurally analogous to `createSizeCapTransform` — receives the completed media block, writes it to the context store, and substitutes a reference in the assistant turn before the turn is appended to `turns.jsonl`. This mirrors the tool-output spill mechanism described in "Tool Interaction Patterns › Tool-output spills".

### Citations and grounding

A unified `citation` content block carries a portable shape — `source` URL or title, `cited_text` span with start/end offsets into the preceding text block, optional `confidence` — plus an opaque `providerDetail` field for provider-specific payload. The provider adapter normalizes the native grounding shape into one or more `citation` blocks attached to the relevant text in the assistant turn; provider-specific fields are preserved in `providerDetail` so policy-bearing payload is not lost, but the portable shape lets a cross-provider consumer render citations without branching on provider identity.

The captured Gemini grounding response (`packages/inference-testing/wire/gemini/google-search-grounding/response.json`) attaches `groundingMetadata` to the candidate rather than producing inline citation blocks. The metadata pairs three documented arrays — `webSearchQueries`, `groundingChunks`, and `groundingSupports` — with a fourth undocumented field, `searchEntryPoint`, whose `renderedContent` is a multi-kilobyte HTML/CSS snippet rendering Google's "Search Suggestions" chip carousel. Each `groundingChunks[].web.uri` is a `vertexaisearch.cloud.google.com/grounding-api-redirect/...` URL, not the source domain; only `title` hints at the underlying domain. The eight-chunk, eight-support shape in this capture is the acceptance test for the Gemini adapter's normalization. The `searchEntryPoint` HTML carries terms-of-service weight — under Google's grounding TOS the chip carousel is what the caller is expected to render to the end user, so it is preserved in `providerDetail` rather than stripped.

Anthropic's citation shape (defined by the Messages API but not yet present in `packages/inference/src/providers/anthropic.ts`) interleaves citations with text content rather than attaching adjunct metadata. Both shapes round-trip through the same portable `citation` block; the adapter is the layer that knows which provider produced which detail.

### Thought signatures

The existing `thinking` content block's optional `signature` field is the canonical opaque-signature slot. Where the provider attaches the signature to a tool call rather than to the thought block — as Gemini does — the adapter populates a parallel optional `signature` field on the `tool_call` block. The signature is carried by whichever block the provider chose, not relocated. The harness round-trip rule is that any block carrying an opaque signature must serialize back to the provider verbatim.

The captured turn-1 response (`packages/inference-testing/wire/gemini/function-calling-thinking/turn-1/response.json`) shows the `thoughtSignature` field on the `functionCall` part; the preceding thought text has no signature of its own. This contradicts the intuitive placement (signature on the reasoning, not on the action), and a naive cross-provider mapping that drops the signature when the thought block has none would lose the model's internal state. The captured turn-2 request (`packages/inference-testing/wire/gemini/function-calling-thinking/turn-2/request.json`) echoes the turn-1 model turn byte-for-byte, including the 408-character signature blob; this is the simulator-backed acceptance fixture for the round-trip rule. The streaming counterpart at `packages/inference-testing/wire/gemini/function-calling-thinking-streaming/turn-1/response.sse` confirms the signature is atomic: it arrives as one complete string inside the same SSE event as its `functionCall` part, not delta-accumulated across multiple events.

The `TokenUsage` shape already exposes a `thinking` field. `thoughtsTokenCount` is charged regardless of `includeThoughts` opt-in — the seven non-thinking captures (`text-non-streaming`, `image-input`, `audio-input`, `video-input`, `pdf-input`, `code-execution`, `google-search-grounding`) all emit non-zero `thoughtsTokenCount`. The opt-in controls whether thought content is returned, not whether tokens are spent. The adapter reports thinking tokens through the existing `TokenUsage.thinking` channel even when no thought block appears in the assistant turn.

The OpenAI-compatible vendors captured in the OpenCode-Zen corpus do not emit an opaque signature for reasoning. None of the five captured models — `kimi-k2.6`, `glm-5.1`, `deepseek-v4-pro`, `qwen3.6-plus`, `mimo-v2-omni` — produces a `signature` or signature-equivalent field on either the reasoning content or the tool call. Reasoning text is treated as plain prose and replayed back into the next turn as plain prose. DeepSeek requires the reasoning text itself to round-trip verbatim on multi-turn assistant messages (see "Reasoning content shape" below); none of these vendors requires anything more than the prose itself. The thought-signature mechanism is therefore Gemini-specific in the day-one corpus, but the optional `signature` slot on `thinking` and `tool_call` blocks is the right shape for accommodating future vendors that introduce one.

### File references and uploads

`file_ref` is a first-class `source` discriminant on per-modality content blocks (see "Media sources" above). The upload flow lives in a separate provider-specific upload module that the harness invokes before issuing the inference call, not in the provider adapter and not in the inference event protocol. The adapter does not perform uploads; the inference event protocol does not carry upload events.

The captured Files API fixture (`packages/inference-testing/wire/gemini/files-api/metadata.json`) is a true two-step capture: step 1 uploads bytes to `POST /upload/v1beta/files` (`packages/inference-testing/wire/gemini/files-api/upload/request.json`), and step 2 references the returned URI in a `generateContent` call (`packages/inference-testing/wire/gemini/files-api/generate/request.json`). The upload response (`packages/inference-testing/wire/gemini/files-api/upload/response.json`) wraps the resource in a top-level `file` object; the returned `uri` carries a 48-hour TTL recorded as `uriContract: "documentary"` in the metadata.

The agent runtime calls the upload module when it needs to convert local bytes into a portable file reference; the resulting `{ uri, mimeType, expirationTime }` is then placed into a content block's `source` field by ordinary code. Token accounting for the file's content cost remains the inference layer's responsibility because the cost is reported in the `generateContent` response — `usageMetadata.promptTokensDetails` in `packages/inference-testing/wire/gemini/files-api/generate/response.json` carries the document modality cost. The `expirationTime` returned by the upload is recorded alongside the reference so that the runtime can re-upload when stale; the inference layer is not responsible for TTL management.

### Provider-specific options

`InferenceOptions` carries an optional `providerOptions?: Record<string, unknown>` field whose value is validated by the active provider's registered arktype validator at the inference-call boundary. Portable knobs — response modalities, thinking budget — get first-class fields on `InferenceOptions`; provider-specific knobs flow through `providerOptions`. The provider registry pattern that already binds adapter pairs to provider identifiers extends to carry an options validator under the same key.

Validation runs at the call boundary, not at type-construction time. When the harness routes an inference call to a provider, it looks up that provider's registered options validator and runs the `providerOptions` payload through it before building the request. A missing validator is a registration error (fail closed; do not pass arbitrary data to a provider that did not declare its option shape). An invalid payload produces an `inference.error` event with a fatal classification rather than a 4xx from the provider.

The captured provider-specific knobs that the Gemini validator must accept on first landing are: `generationConfig.thinkingConfig` (budget and `includeThoughts`; `packages/inference-testing/wire/gemini/function-calling-thinking/turn-1/request.json`), `responseModalities` (`packages/inference-testing/wire/gemini/image-output/request.json`), the server-side tool toggles `tools: [{ codeExecution: {} }]` and `tools: [{ googleSearch: {} }]` (`packages/inference-testing/wire/gemini/code-execution/request.json` and `packages/inference-testing/wire/gemini/google-search-grounding/request.json`), and `toolConfig.functionCallingConfig` (`packages/inference-testing/wire/gemini/function-calling-multi-turn/turn-1/request.json`).

### Reasoning content shape

On the OpenAI-compatible side, reasoning is surfaced as plain prose carried on a dedicated field of the message envelope (non-streaming) or the delta (streaming). The portable shape the taxonomy speaks is a `thinking` content block with prose text and no signature, sourced from one of three observed wire fields: `choices[0].message.reasoning_content`, `choices[0].message.reasoning`, or `choices[0].message.reasoning_details[].text`. Adapter logic reads whichever field the relay populated and writes a single `thinking` block per assistant turn; on the way back to the provider for multi-turn assistant messages, the adapter writes the prose into the same field it was read from. Reasoning prose round-trips verbatim where the upstream contract requires it, and is dropped where the upstream contract does not.

Four of the five captured OpenCode-Zen models use `message.reasoning_content` uniformly: `glm-5.1` (`packages/inference-testing/wire/opencode-zen/glm-5.1/reasoning-non-streaming/response.json`), `deepseek-v4-pro` (`packages/inference-testing/wire/opencode-zen/deepseek-v4-pro/reasoning-non-streaming/response.json`), `qwen3.6-plus` (`packages/inference-testing/wire/opencode-zen/qwen3.6-plus/reasoning-non-streaming/response.json`), and `mimo-v2-omni` (`packages/inference-testing/wire/opencode-zen/mimo-v2-omni/reasoning-non-streaming/response.json`). The fifth, `kimi-k2.6`, splits depending on which upstream backend the relay routes to: the Moonshot AI path used for text and vision (`packages/inference-testing/wire/opencode-zen/kimi-k2.6/text-non-streaming/response.json`) emits `message.reasoning` plus a parallel `reasoning_details: [{type: "reasoning.text", text, format, index}]` array, while the Fireworks path used for function calling and reasoning (`packages/inference-testing/wire/opencode-zen/kimi-k2.6/reasoning-non-streaming/response.json`) emits `message.reasoning_content`. The same model can produce different reasoning-field names across capabilities; the adapter must read all three names and not assume a per-model fixed shape.

The streaming counterpart of every captured reasoning surface uses the same field name on the delta. `choices[0].delta.reasoning_content` appears in the SSE captures at `packages/inference-testing/wire/opencode-zen/glm-5.1/reasoning-streaming/response.sse`, `deepseek-v4-pro/reasoning-streaming/response.sse`, `qwen3.6-plus/reasoning-streaming/response.sse`, `mimo-v2-omni/reasoning-streaming/response.sse`, and `kimi-k2.6/reasoning-streaming/response.sse`. The OpenAI Chat Completions reference does not specify reasoning fields at all; the variants are vendor extensions normalised onto a common name by the relay for four of the five models. The presence or absence of a sibling `content` key during the reasoning phase varies by vendor: `deepseek-v4-pro` always emits `content: null` alongside `reasoning_content`, `mimo-v2-omni` always emits `content: null` along with explicit `role: null` and `tool_calls: null` slots, `qwen3.6-plus` emits `content: null` on the first delta and drops it later, while `kimi-k2.6` and `glm-5.1` omit the `content` key entirely during reasoning deltas. A consumer that distinguishes "reasoning chunk" from "answer chunk" by key presence rather than by which key carries the non-null delta value will misclassify chunks across vendors. The correct rule is "the chunk's delta payload is whatever key holds a non-null string"; both `reasoning_content` and `content` may appear with `null` placeholders.

DeepSeek's documented contract additionally requires `reasoning_content` to round-trip verbatim on multi-turn assistant messages (`packages/inference-testing/wire/opencode-zen/deepseek-v4-pro/function-calling-multi-turn/turn-2/request.json`); omitting it produces an HTTP 400. The portable rule is that the adapter forwards the entire turn-1 assistant message into turn-2 unchanged, preserving any vendor-specific reasoning slots alongside the standard `role`, `content`, and `tool_calls` fields. The reasoning prose is part of the assistant turn, not an out-of-band artefact.

### Tool-call envelope variants

The OpenAI Chat Completions tool-call shape leaves several details unspecified — `tool_calls[].index`, the format of `tool_calls[].id`, the value of `choices[0].message.content` alongside a tool call, and whether vendor-specific slots may appear on the message — and the captured OpenCode-Zen vendors vary on every one of those axes. The adapter normalises the variation into a single `tool_call` content block and a single `text` block (which may be empty) on the way in; the assistant turn on the way out preserves the vendor-specific slots verbatim by carrying them as opaque round-trip payload.

`tool_calls[].index` is present on three of the five captured vendors and absent on two: present on `kimi-k2.6` (`packages/inference-testing/wire/opencode-zen/kimi-k2.6/function-calling-single/response.json`), `deepseek-v4-pro` (`packages/inference-testing/wire/opencode-zen/deepseek-v4-pro/function-calling-single/response.json`), and `qwen3.6-plus` (`packages/inference-testing/wire/opencode-zen/qwen3.6-plus/function-calling-single/response.json`); absent on `glm-5.1` (`packages/inference-testing/wire/opencode-zen/glm-5.1/function-calling-single/response.json`) and `mimo-v2-omni` (`packages/inference-testing/wire/opencode-zen/mimo-v2-omni/function-calling-single/response.json`). The adapter cannot rely on `index` being present and must derive call ordering from the array order itself.

`tool_calls[].id` varies across three forms with no shared structure: long opaque `call_<hex>` (`deepseek-v4-pro`, `qwen3.6-plus`, `mimo-v2-omni`), short opaque `call_<4-hex>` (`glm-5.1`), and function-name-keyed `functions.<name>:<index>` (`kimi-k2.6` on the Fireworks path). All three are valid OpenAI tool-call IDs — the spec is opaque on the format — but consumers that pattern-match on `call_` will not recognise the Kimi form. The adapter treats the id as an opaque correlator: the turn-2 `role: "tool"` response keys back to the same string the model produced, regardless of its format.

`choices[0].message.content` alongside a tool call is the empty string `""` on four of the five vendors and `null` on `mimo-v2-omni` (`packages/inference-testing/wire/opencode-zen/mimo-v2-omni/function-calling-single/response.json`). The adapter normalises both to "no text content" on the assistant turn; consumers that test `content === ""` to detect a tool-only turn will miss the MiMo case.

Two vendors emit additional `null` slots on the message envelope that are not part of the OpenAI shape: `kimi-k2.6` on the Fireworks path adds a sibling `tool_calls[].name: null` alongside the nested `function.name`, and `glm-5.1` adds `choices[0].message.name: null` on every assistant turn including plain text turns. The adapter must preserve these slots when echoing the assistant message back in a multi-turn capture; the captured turn-2 requests at `packages/inference-testing/wire/opencode-zen/glm-5.1/function-calling-multi-turn/turn-2/request.json` and `packages/inference-testing/wire/opencode-zen/kimi-k2.6/function-calling-multi-turn/turn-2/request.json` echo the vendor-specific slots verbatim alongside the standard fields.

### Streaming terminators and trailers

The OpenAI Chat Completions SSE specification documents `data: [DONE]\n\n` as the stream terminator. The OpenCode-Zen relay emits one further `data:` event after `[DONE]` on every captured streaming response: a `data: {"choices":[],"cost":"0"}\n\n` chunk reporting a stringified cost. The adapter treats `[DONE]` as the logical end of stream and the post-`[DONE]` trailer as an optional cost-reporting suffix; consumers that terminate parsing at `[DONE]` will not break, but consumers that continue reading past `[DONE]` must tolerate at least one additional `data:` event with an empty `choices` array.

The trailer is universal across the corpus: text-streaming and reasoning-streaming captures for every one of the five models end with the same byte sequence. The text-streaming evidence is at `packages/inference-testing/wire/opencode-zen/kimi-k2.6/text-streaming/response.sse`, `glm-5.1/text-streaming/response.sse`, `deepseek-v4-pro/text-streaming/response.sse`, `qwen3.6-plus/text-streaming/response.sse`, and `mimo-v2-omni/text-streaming/response.sse`; the reasoning-streaming evidence is at the matching `<model>/reasoning-streaming/response.sse` paths. The corresponding response headers carry `content-type: text/event-stream` (with minor charset and spacing variation across vendors that does not affect SSE parsing).

The terminal usage chunk before `[DONE]` carries an empty `choices` array and a populated `usage` block on every model; on three of the five vendors (`deepseek-v4-pro`, `qwen3.6-plus`, `mimo-v2-omni`) it additionally carries `usage.completion_tokens_details.reasoning_tokens`. The adapter reads usage from this chunk, then emits `[DONE]` as the stream terminator and discards the cost trailer; the portable inference event protocol does not surface the relay's per-request cost report.

### Forward references

INTR-79 retrofits this taxonomy across `@intx/types` (extending the `ContentBlock` union with per-modality blocks, adding `source` discriminants, adding the `citation` block, threading `providerOptions` through `InferenceOptions`), `@intx/inference-testing` (the simulator understands the new block shapes and the file-reference source, replays the OpenCode-Zen SSE captures including the post-`[DONE]` cost trailer, and accepts the three reasoning-field names enumerated in "Reasoning content shape"), and the existing OpenAI and Anthropic adapters (they round-trip the new blocks correctly, preserving opaque signatures and provider detail, and the OpenAI-compatible adapter normalises the `reasoning_content` / `reasoning` / `reasoning_details` variants onto a single `thinking` content block while forwarding reasoning prose verbatim where the upstream contract requires it). INTR-80 lands the Gemini adapter end-to-end against the resulting surface.
