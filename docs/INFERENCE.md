...
This section commits to the shape of the cross-provider content taxonomy that the inference layer will speak after the Gemini adapter lands. The day-one taxonomy described above ("Message Format › Content Types") covers text, thinking, image input, tool call, and tool result. That set is sufficient for Anthropic and OpenAI-compatible providers but does not accommodate the additional surfaces that Gemini exposes: audio, video, and PDF inputs; image output; thought signatures; Google Search grounding citations; the Files API; and request-shape knobs that have no portable analogue (response modalities, thinking budgets, server-side tools like code execution).

The decisions below were informed by a 2026-05-20 capture campaign against the Gemini Developer API v1beta surface. Twelve capability slices were recorded under `packages/inference-testing/wire/gemini/`, each comprising the live request bytes, response bytes (or SSE stream), headers, and a `metadata.json` index. The per-capability observed-vs-documented analysis lives in `docs/GEMINI_DISCOVERY.md`; the captures themselves are the ground truth. Where prior planning guidance conflicts with the captured bytes, the captures win — every decision below cites at least one capture path as evidence.

This section is design, not implementation. INTR-79 retrofits the taxonomy across `@intx/types`, `@intx/inference-testing`, and the existing OpenAI and Anthropic adapters. INTR-80 lands the Gemini adapter end-to-end against this taxonomy. The six `#### Decision:` subheadings below are named so those issues can cite them directly.

### Capability surface

The union of capabilities the taxonomy must accommodate, drawn from the three day-one providers (Anthropic, OpenAI-compatible, Gemini), is wider than the day-one content-block set. Inputs include text, base64 inline images, base64 inline audio, base64 inline video, base64 inline PDF (and other documents), and file references obtained via a prior upload. Outputs include text, thinking with optional opaque signatures, tool calls, tool results, generated images, and grounded text with citation spans linking back to web sources. Request-shape inputs include tool definitions, thinking configuration, response modalities, server-side tool toggles (code execution, Google Search), and provider-specific knobs that have no portable analogue.

The Gemini capture corpus at `packages/inference-testing/wire/gemini/` is the concrete evidence base: `text-non-streaming`, `text-streaming`, `function-calling-multi-turn`, `function-calling-thinking`, `image-input`, `image-output`, `audio-input`, `video-input`, `pdf-input`, `code-execution`, `google-search-grounding`, and `files-api`. Decisions below cite specific paths under that tree.

### Design question 1: Media source representation

How should the content-block taxonomy represent media inputs (image, audio, video, document) when sources can be inline base64 bytes or a reference to a previously uploaded file? Two shapes were considered: per-modality blocks (`image`, `audio`, `video`, `document`) each carrying a `source` discriminant that names inline-bytes versus file-reference, or a single generic `media` block with a `kind` field.

The captured Gemini requests place the modality and the source on independent axes. An inline image (`packages/inference-testing/wire/gemini/image-input/request.json`), inline audio (`packages/inference-testing/wire/gemini/audio-input/request.json`), inline video (`packages/inference-testing/wire/gemini/video-input/request.json`), and inline PDF (`packages/inference-testing/wire/gemini/pdf-input/request.json`) all share the same wire shape: a part with `inlineData: {mimeType, data}`. A file-reference part (`packages/inference-testing/wire/gemini/files-api/generate/request.json`) uses `fileData: {mimeType, fileUri}` regardless of the underlying modality. The `promptTokensDetails` arrays in the captured responses label modalities explicitly (`TEXT`, `IMAGE`, `AUDIO`, `VIDEO`, `DOCUMENT`), confirming that the provider itself models modality as a first-class attribute orthogonal to source.

Per-modality blocks keep the taxonomy aligned with how callers reason about content (an image is not interchangeable with a PDF at the application layer), allow per-modality fields without union sprawl (an image may carry alt text; a video may carry a time range), and preserve the existing `image` block. A shared `source` discriminant unifies the inline-vs-reference axis without forcing four parallel pair-types.

#### Decision: Per-modality content blocks (`image`, `audio`, `video`, `document`) share a discriminated `source` field naming `inline` (base64 bytes plus MIME type) or `file_ref` (provider-issued reference plus MIME type).

The existing `image` block (`type: "image"`, base64 inline) becomes the inline case of the new shape. New per-modality blocks for audio, video, and document follow the same pattern. The `source` discriminant carries the upload-flow indirection without coupling the modality vocabulary to it, and the `promptTokensDetails` modality labels in `packages/inference-testing/wire/gemini/video-input/response.json` (which splits a single mp4 part into `VIDEO` and `AUDIO` cost lines) confirm that the provider already treats modality as a billing-relevant axis the taxonomy must surface.

### Design question 2: Image-output streaming

How should the event protocol expose generated images? The day-one event surface emits text and thinking as deltas (`inference.text.delta`, `inference.thinking.delta`); image output has no precedent. Two shapes were considered: emit an `inference.media.complete` event carrying the finished blob (and let the harness's tool-output spill path or an equivalent transforms hand-off own the bytes), or accumulate base64 deltas analogous to text.

The captured Gemini image-output response (`packages/inference-testing/wire/gemini/image-output/response.json`) is not a streaming endpoint — it is a single non-streaming `generateContent` call that returns one candidate with two parts: a text caption followed by an `inlineData` part with `mimeType: "image/png"` and a base64 payload approximately 1.17 MB on disk (per `packages/inference-testing/wire/gemini/image-output/metadata.json`). The image is fully formed in one response; the provider does not chunk it.

This rules out the delta-accumulation shape on the merits the provider gives us: there are no deltas to accumulate. It also surfaces a downstream concern — a multi-megabyte base64 payload sitting inline in a content block bloats every subsequent prompt that replays the assistant turn. The tool-output spill mechanism described in "Tool Interaction Patterns › Tool-output spills" already solves the equivalent problem for tool results by spilling to `tool-output:///{callId}` and substituting a pointer; image output deserves the same treatment via the transforms pipeline so that the inline content block is replaced with a reference once the cycle is complete.

#### Decision: Image output is emitted as a single `inference.media.complete` event carrying the modality, MIME type, and bytes (or a context-store reference if a transform has spilled them); no delta accumulation.

The streaming harness emits `inference.media.complete` at the point the part is finalized, regardless of whether the underlying provider call is streamed or non-streamed. A media-spill transform — structurally analogous to `createSizeCapTransform` — receives the completed media block, writes it to the context store, and substitutes a reference in the assistant turn before the turn is appended to `turns.jsonl`. The captured 1.17 MB payload in `packages/inference-testing/wire/gemini/image-output/response.json` is the concrete justification: inlining bytes of that size in every replay is unaffordable, and the transforms pipeline is the layer that already owns the inline-versus-spilled trade-off.

### Design question 3: Citation/grounding shape

How should grounded responses (text with citations linking back to source material) be represented? Two shapes were considered: a unified `citation` block carrying a portable shape plus an opaque field for provider-specific detail, or per-provider citation blocks.

The captured Gemini grounding response (`packages/inference-testing/wire/gemini/google-search-grounding/response.json`) attaches `groundingMetadata` to the candidate rather than producing inline citation blocks. The metadata pairs three documented arrays — `webSearchQueries`, `groundingChunks`, and `groundingSupports` — with a fourth undocumented field, `searchEntryPoint`, whose `renderedContent` is a multi-kilobyte HTML/CSS snippet that renders Google's "Search Suggestions" chip carousel. Each `groundingChunks[].web.uri` is a `vertexaisearch.cloud.google.com/grounding-api-redirect/...` URL, not the source domain; only `title` hints at the underlying domain. Supports pair 1:1 with chunks in this capture but the data model permits a support to reference multiple chunks via `groundingChunkIndices`, and `confidenceScores` is optional in practice (absent from this capture despite the docs implying it is typical).

Anthropic's citation shape (not yet present in `packages/inference/src/providers/anthropic.ts` but defined by the Messages API) carries inline citation blocks with `cited_text`, `document_index`, and per-source-type detail. The two shapes do not align: Gemini attaches grounding to the candidate as adjunct metadata, while Anthropic interleaves citations with text content. A unified portable shape must handle both, and per-provider blocks would force every consumer to branch on provider identity.

Two further constraints emerged from the capture. First, `searchEntryPoint` carries terms-of-service weight — under Google's grounding TOS the chip carousel is what the caller is expected to render to the end user, so stripping it is a policy decision rather than a free choice. Second, `groundingChunks[].web.uri` redirect URLs are documentary artifacts: simulator consumers and audit replays read the captured bytes, not live URLs.

#### Decision: A unified `citation` content block carries a portable shape (`source` URL or title, `cited_text` span with start/end offsets into the preceding text block, optional `confidence`) plus an opaque `providerDetail` field for provider-specific payload (Gemini's redirect URI and `searchEntryPoint` snippet; Anthropic's `document_index` and source-type discriminant).

The provider adapter is responsible for normalizing the provider's native grounding shape into one or more `citation` blocks attached to the relevant text in the assistant turn. The full provider payload — including Gemini's `searchEntryPoint` HTML and `webSearchQueries` array — is preserved in `providerDetail` so policy-bearing fields are not lost, but the portable `source`/`cited_text`/`confidence` shape lets a cross-provider consumer render citations without branching on provider identity. The captured eight-chunk, eight-support shape in `packages/inference-testing/wire/gemini/google-search-grounding/response.json` is the concrete acceptance test for the Gemini adapter's normalization.

### Design question 4: Thought signatures

How should opaque thought signatures — the base64 blobs Gemini emits to preserve the model's internal reasoning state across turns — be represented? Two shapes were considered: extend the existing `thinking` block with an opaque `signature` field, or introduce a new opaque block.

The existing `thinking` content block in `packages/types/src/runtime.ts` already carries an optional `signature` field that the Anthropic adapter populates for extended-thinking continuity. Gemini's signature is more constrained but structurally analogous, with three properties the captures pin down precisely.

First, the signature attaches to the `functionCall` part, not to the thought (`text` with `thought: true`) part. The captured turn-1 response (`packages/inference-testing/wire/gemini/function-calling-thinking/turn-1/response.json`) shows the `thoughtSignature` field on the `functionCall`; the preceding thought text has no signature of its own. This contradicts the intuitive placement (signature on the reasoning, not on the action) and a naive cross-provider mapping would lose the signature entirely.

Second, the signature must round-trip verbatim. The captured turn-2 request (`packages/inference-testing/wire/gemini/function-calling-thinking/turn-2/request.json`) echoes the turn-1 model turn byte-for-byte, including the 408-character signature blob. A caller that reconstructs the model turn from a structured representation (e.g., `{name, args}` only on the tool call) will silently strip the signature and lose the model's internal state.

Third, `thoughtsTokenCount` is charged regardless of `includeThoughts` opt-in. Seven non-thinking captures (`text-non-streaming`, `image-input`, `audio-input`, `video-input`, `pdf-input`, `code-execution`, `google-search-grounding`) still emitted non-zero `thoughtsTokenCount`. The opt-in controls whether thought content is returned, not whether tokens are spent. The TokenUsage shape already exposes a `thinking` field, but token accounting layers must surface it even when no thought block appears in the assistant turn.

#### Decision: Extend the existing `thinking` block's optional `signature` field rather than introduce a new opaque block, but attach the signature to the action it travels with (tool call) when the provider does, and enforce byte-for-byte round-trip in the harness.

The thinking block's `signature` remains optional and opaque. Where the provider attaches the signature to a tool call (Gemini), the adapter populates a parallel optional `signature` field on the `tool_call` block — the signature is carried by whichever block the provider chose, not relocated to the thinking block. The harness round-trip rule is that any block carrying an opaque signature must serialize back to the provider verbatim; the simulator-backed tests in `packages/inference-testing/wire/gemini/function-calling-thinking/turn-2/request.json` are the acceptance fixture. The undocumented behaviour that `thoughtsTokenCount` is charged without opt-in is preserved as-is in the existing `TokenUsage.thinking` channel; no taxonomy change required there beyond ensuring the adapter reports it.

### Design question 5: Files API source kind

How should the content-block taxonomy represent a reference to a previously uploaded file, and where in the agent runtime does the upload flow live? Decision 1 above fixes the source kind (`file_ref`); this question fixes the upload-flow location.

The captured Files API fixture (`packages/inference-testing/wire/gemini/files-api/metadata.json`) is a true two-step capture: step 1 uploads bytes to `POST /upload/v1beta/files` (`packages/inference-testing/wire/gemini/files-api/upload/request.json` plus `request-headers.json`), and step 2 references the returned URI in a `generateContent` call (`packages/inference-testing/wire/gemini/files-api/generate/request.json`). The upload response (`packages/inference-testing/wire/gemini/files-api/upload/response.json`) wraps the resource in a top-level `file` object whose `uri` is documentary: per `metadata.json`'s `uri-contract: "documentary"` and `uri-ttl-hours: 48`, the URI expires 48 hours after upload, and simulator consumers read fixture bytes, not URI reachability.

This separates two concerns. The taxonomy concern — how a content block names a file reference — is settled by Decision 1: a per-modality block with `source: { kind: "file_ref", uri, mimeType }`. The runtime concern — where the upload happens, who owns the URI lifecycle, and how the inference layer is told the upload is done — is distinct. Three candidate locations were considered: the provider adapter (the adapter receives raw bytes and uploads them transparently before the inference call), the harness (the harness uploads bytes as a step in agent-runtime input handling and hands the inference layer a finished file reference), or a separate package (a `@intx/uploads` package that owns provider-specific upload protocols).

Putting the upload flow in the adapter forces every adapter to implement an upload protocol even though most providers do not have one. Putting it in the harness keeps the inference layer focused on inference but couples the harness to provider-specific upload semantics. A separate package matches the constraint-ownership principle: the upload protocol is provider-specific knowledge that does not belong in the harness's general agent-lifecycle layer, and it is not part of the inference event protocol.

#### Decision: `file_ref` is a first-class `source` discriminant on per-modality content blocks (Decision 1); the upload flow lives in a separate provider-specific upload module that the harness invokes before issuing the inference call, not in the provider adapter and not in the inference event protocol.

The agent runtime calls the upload module when it needs to convert local bytes into a portable file reference; the resulting `{ uri, mimeType, expirationTime }` is then placed into a content block's `source` field by ordinary code. The inference adapter does not perform uploads, and the inference event protocol does not carry upload events. Token accounting for the file's content cost remains the inference layer's responsibility because the cost is reported in the `generateContent` response (the captured `usageMetadata.promptTokensDetails` in `packages/inference-testing/wire/gemini/files-api/generate/response.json` carries the document modality cost). The `expirationTime` returned by the upload (48 hours per the captured fixture) is recorded alongside the reference so that the runtime can re-upload when stale; the inference layer is not responsible for TTL management.

### Design question 6: providerOptions validation

How does a portable caller pass provider-specific request knobs that have no portable analogue on `InferenceOptions`? The captures surfaced several such knobs: Gemini's `generationConfig.thinkingConfig.thinkingBudget` and `includeThoughts` (`packages/inference-testing/wire/gemini/function-calling-thinking/turn-1/request.json`), `responseModalities: ["TEXT", "IMAGE"]` (`packages/inference-testing/wire/gemini/image-output/request.json`), server-side tool toggles `tools: [{ codeExecution: {} }]` and `tools: [{ googleSearch: {} }]` (`packages/inference-testing/wire/gemini/code-execution/request.json` and `packages/inference-testing/wire/gemini/google-search-grounding/request.json`), and `toolConfig.functionCallingConfig.mode` with `allowedFunctionNames` (`packages/inference-testing/wire/gemini/function-calling-multi-turn/turn-1/request.json`).

Some of these have portable analogues that should land on `InferenceOptions` directly: response modalities and thinking budget map cleanly onto cross-provider concepts (the existing `thinking` option already exists). Others — `googleSearch`, `codeExecution`, `allowedFunctionNames` — are intrinsically provider-specific and have no useful portable shape. A single untyped `providerOptions?: Record<string, unknown>` is the obvious escape hatch but defeats the package's `arktype`-everywhere validation discipline (see CONVENTIONS.md § Type System Patterns › Runtime Validation with arktype): an unvalidated `unknown` would be the only such surface in the package.

Two alternatives were considered: a narrow per-provider typed record (`providerOptions?: { anthropic?: AnthropicOptions; openai?: OpenAIOptions; gemini?: GeminiOptions }`) versus a registered validator map keyed by provider identifier. The typed record couples `@intx/types` to every provider's option shape and prevents external provider registrations from extending it. The registered validator map matches the existing provider registry pattern: providers register an adapter pair (request builder, response parser) at startup; they can also register an `arktype` validator for their options block under the same key.

#### Decision: `InferenceOptions` carries an optional `providerOptions?: Record<string, unknown>` field whose value is validated by the active provider's registered arktype validator at the inference-call boundary; portable knobs (response modalities, thinking budget) get first-class fields on `InferenceOptions`, while provider-specific knobs flow through `providerOptions`.

Validation is enforced at the call boundary, not at type-construction time. When the harness routes an inference call to a provider, it looks up that provider's registered options validator and runs the `providerOptions` payload through it before building the request. A missing validator is a registration error (fail closed, do not pass arbitrary data to a provider that did not declare its option shape). An invalid payload produces an `inference.error` event with a fatal classification rather than a 4xx from the provider. The captured provider-specific knobs above are the acceptance vocabulary the Gemini validator must accept on first landing.

### Forward references

INTR-79 retrofits this taxonomy across `@intx/types` (extending the `ContentBlock` union with per-modality blocks, adding `source` discriminants, adding the `citation` block, threading `providerOptions` through `InferenceOptions`), `@intx/inference-testing` (the simulator must understand the new block shapes and the file-reference source), and the existing OpenAI and Anthropic adapters (they must round-trip the new blocks correctly, preserving opaque signatures and provider detail). INTR-80 lands the Gemini adapter end-to-end against the resulting surface. Each `#### Decision:` subheading above is named so those issues can cite it directly — for example, "INTR-79 implements Decision 1 (per-modality blocks with shared source discriminant)".
