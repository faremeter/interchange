...
Before creating a deployment, the Hub records a `workflow_probe` and matches tenant and Hub probe policy against a separately configured probe-provisioner list. Every match is passed to the probe chooser together with the authenticated placement principal and effective capability policy; the chooser selects the capacity that runs the probe, and probing never runs on the Hub or an ambient sidecar. After freezing the result, the Hub folds requirements from the top-level definition and its inline loops, trigger bodies, and child workflows, matches the separately configured deployment-provisioner list, and passes those matches together with the same tenant and placement principal and the final effective policy to the deployment chooser. It adopts the probe capacity when both choosers selected the same provisioner binding and any claimed host satisfies the final policy; otherwise it destroys that capacity and creates a normal pending allocation through the selected deployment provisioner. No match fails closed. The default chooser selects the first match in registration order, while operator compositions may inject another asynchronous policy.

Every deployment is anchored to a durable sidecar allocation and routed through its authenticated generation. The allocation persists the placement principal separately from workflow source authority and passes only that principal id, tenant id, and allocation lifecycle data to `ensure()`. Provisioners decide internally whether backing capacity is created, isolated, shared, or reused; a provisioner using pre-existing capacity owns its per-capacity capabilities and atomic claim. The Hub relies only on the provisioner's declared guarantees. The Hub has no shared or non-provisioner sidecar identity, and every probe, deployment, trigger, signal, and workflow-state write revalidates the allocation generation.
...
Reclaiming a validation-rejected pack is separate and immediate: `receivePackObjects` and `applyPack` unpublish it at the moment of rejection rather than waiting for the next GC threshold, so a flood of bad packs cannot accumulate in the window before reclaim. Once ref promotion begins, failures retain the already-durable pack because the ref update may have become observable.
