...
The deployment request stays open while the first registered provisioner matching tenant and Hub probe policy starts probe capacity and the exact authenticated generation returns `workflow.probe.result`. The Hub freezes that result and selects the first provisioner matching the final capability policy. If the same binding still matches, the Hub atomically creates the normal anchor run, frozen launch spec, and an allocated `sidecar_allocation` that adopts the connected capacity. Otherwise it destroys the probe capacity and creates a normal pending allocation for the selected binding. Only then does the deployment exist.
