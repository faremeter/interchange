...
A terminal `destroy_failed` allocation records a permanent cleanup failure where infrastructure may remain. The allocation store preserves the sidecar id, external reference, and exclusive reservation of that sidecar id for manual cleanup. It records the provider's failure code and message, fails any remaining live runs and unsettled dispatches, and clears scheduled reconciliation. The allocation cannot dispatch work or create a replacement, and the deployment list reports `destroy_failed` explicitly.
