...
Every replacement increments `generation`. The generation and placement principal are persisted before the provisioner is called; the principal appears only in `ensure`, while the generation appears in ensure/destroy requests and the sidecar's authenticated identity. This preserves principal-scoped placement across reconciliation and replacement without making principal context necessary for cleanup. Provisioners must make ensure/destroy idempotent and reject a generation older than one they have already observed. The Hub separately records `ensure_accepted_generation`; registration may occur while capacity is converging, but readiness and routing require the exact generation to be accepted.
