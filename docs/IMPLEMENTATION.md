...
Allocation reconciliation uses eight concurrent slots by default (`sidecarAllocationConcurrency` on `createHubServer`). Each free slot can claim work without waiting for a batch of other slots to finish. An admission limit equal to that concurrency setting (`maxConcurrentClaims` when constructing the reconciler directly) covers pending claims, active reconciliation, and allocations with retained readiness/recovery queries, lease-validation queries, lease renewals, or connection-event writes. Capacity is reserved before issuing a claim and retained through reconciliation, without a gap at the claim response. After cancellation or timeout, unfinished tracked work continues to count until it settles; active allocations count once even when they have pending queries or writes. Overlapping queries keep the allocation excluded and its capacity reserved until every query settles, including renewals that outlive a completed reconciliation. When the limit is full, new claims wait for a later scheduler poll. Late claim results never start reconciliation. Claims exclude allocations still executing in the same Hub and allocations with pending connection-event writes. Waiting to enter or finish initialization through that queue obeys cancellation, lease expiry, and the operation deadline. A cancelled waiter releases its reconciliation slot; its queued callback cannot run later, while already-started database writes retain their ordering until they settle. Unrelated allocations can proceed during that wait while admission capacity remains available. Probe cleanup, workflow dispatch, and connection repair run on independent schedules. Periodic dispatch drains directly, independently of the guarded drain used by enqueue notifications. Both drains exclude dispatches whose earlier delivery work is still executing, even after the database lease expires. Once a claim returns, delivery and retry writes are bounded by the remaining delivery lease (30 seconds by default), counted conservatively from the start of the claim. Expiry frees the drain while unfinished I/O keeps that dispatch excluded until it settles; cancellation checks after address, identity, and sender-key lookups prevent late sends. Provider, allocation claim, lease-validation, and allocation connection-wait deadlines share the `sidecarOperationTimeoutMs` setting.
