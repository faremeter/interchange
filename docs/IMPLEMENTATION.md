...
A non-retryable destroy rejection moves a `releasing` or `replacing` allocation to terminal `destroy_failed`. The Hub records the provider's failure code and message, fails any remaining live runs and unsettled dispatches, retires the allocation connection, and stops automatic reconciliation, including after restart. Retryable destroy rejections and thrown errors continue to back off and retry. Infrastructure may remain after `destroy_failed`: the allocation preserves its sidecar id, external reference, and exclusive reservation of that sidecar id for operator cleanup directly against the provider (there is no in-Hub cleanup action for this state). It cannot dispatch work or create a replacement. The deployment list reports `destroy_failed` explicitly.
...
The Hub never persists raw sidecar bearer tokens; it stores only their hashes. Every stored sidecar token hash belongs to a current workflow probe or allocation; an identity without either active owner is rejected. If the Hub restarts during a probe, startup marks its temporary capacity for release; background reconciliation performs and retries destruction. If it restarts after binding an allocation generation but before recording ensure acceptance, it treats that outcome as uncertain, fences the generation, destroys it idempotently, and mints a replacement identity. On Hub startup, interrupted probes are durably marked for cleanup before active allocation fences are rebuilt; no provider call is awaited during this initialization.
...
Provisioner `ensure` and `destroy` calls have a 120-second default deadline, configurable through `operationTimeoutMs` on the allocation reconciler and workflow allocation service. The Hub supplies an abort signal and stops waiting at the deadline even if a provisioner ignores cancellation. An ensure timeout is an uncertain outcome requiring fencing and cleanup; a destroy timeout leaves cleanup pending and cannot authorize replacement. A late provider operation must still respect the provisioner generation fence.

One lease heartbeat spans the entire claimed reconciliation, including database transitions between provider calls and the final readiness write. Changing stages does not restart the renewal timer, and a pending renewal remains owned by that claim. Completion or cancellation makes its late response inert. Lease-validation query errors follow the lease-loss retry path. Readiness queries before connection waits and after ensure acceptance also obey the operation deadline and lease cancellation.

Several lookups can fail or stall without establishing that a worker is gone. Each is inconclusive and schedules a retry rather than a release:
...
Release and replacement transactions use the bounded per-allocation connection queue. Cancellation, lease expiry, or the operation deadline frees the scheduler slot while the pending transaction keeps the allocation excluded from new claims. Claims recheck local work after returning so an older exclusion snapshot cannot overtake pending cleanup, reconciliation, or queries. A late claim that finds such work returns without parking and lets its lease expire. Lease loss, renewal errors, and lease-validation errors stop reconciliation without a parking write. The heartbeat stops and the existing durable lease and retry schedule govern the next claim; these paths do not add a fresh retry backoff. Unfinished tracked queries and writes keep their allocation excluded until they settle. A late committed transition advances the router's generation fence before the queued work settles and its exclusion clears.

Allocation reconciliation uses eight concurrent slots by default (`sidecarAllocationConcurrency` on `createHubServer`). Each free slot can claim work without waiting for a batch of other slots to finish. An admission limit equal to that concurrency setting (`maxConcurrentClaims` when constructing the reconciler directly) covers pending claims, active reconciliation, and allocations with retained readiness/recovery queries, lease-validation queries, lease renewals, or connection-event writes. When the limit is full, new claims wait for a later scheduler poll.

- Capacity is reserved before issuing a claim and retained through reconciliation, without a gap at the claim response. After cancellation or timeout, unfinished tracked work continues to count until it settles; active allocations count once even when they have pending queries or writes.
- Overlapping queries keep the allocation excluded and its capacity reserved until every query settles, including renewals that outlive a completed reconciliation.
- Claims exclude allocations still executing in the same Hub and allocations with pending connection-event writes. Late claim results never start reconciliation.
- Waiting to enter or finish initialization through that queue obeys cancellation, lease expiry, and the operation deadline. A cancelled waiter releases its reconciliation slot; its queued callback cannot run later, while already-started database writes retain their ordering until they settle. Unrelated allocations can proceed during that wait while admission capacity remains available.

Probe cleanup, workflow dispatch, and connection repair run on independent schedules. Periodic dispatch drains directly, independently of the guarded drain used by enqueue notifications. Both drains share an admission limit of eight unfinished attempts by default (`maxConcurrentDispatches` on `createWorkflowDispatchService`). Pending claims reserve capacity before querying; delivery and retry writes retain that capacity until they settle, even after timeout. Both drains exclude dispatches whose earlier delivery work is still executing, even after the database lease expires. Once a claim returns, delivery and retry writes are bounded by the remaining delivery lease (30 seconds by default), counted conservatively from the start of the claim. Expiry frees the drain while unfinished I/O keeps that dispatch excluded until it settles; cancellation checks after address, identity, and sender-key lookups prevent late sends. Provider, allocation claim, lease-validation, and allocation connection-wait deadlines share the `sidecarOperationTimeoutMs` setting.

The allocation reconciler uses database leases and retry timestamps rather than process-local jobs. Within a Hub, connection events and ordinary reconciliation completion share a per-allocation queue. A disconnect cancels the active attempt and invalidates its existing database lease. The readiness pass consumes connection notifications before starting initialization. A later connect schedules an immediate follow-up after successful initialization; failed initialization retains its failure details and retry backoff. Losing or failing to renew a lease cancels initialization. A local watchdog also cancels it if renewal never settles before the last confirmed lease expires. Successful renewals advance that deadline, while late results from cancelled or completed operations are ignored. Restore and deploy sends check cancellation, and publishing the initialization key requires the current unexpired lease. Workflow initialization has no total deadline: each restore pack acknowledgement has a 120-second default timeout, and the deploy acknowledgement has a 30-second default timeout. Restore or preparation failures can retry the same generation; failures after the deploy frame was sent remain uncertain and require fencing and release or replacement. Repository preparation and database work have no separate initialization deadline and can occupy a reconciliation slot while the lease remains current. Probe cleanup callers in one Hub share an in-flight operation and reread its current state before destruction. The existing `releasing` status is the durable cleanup obligation; the independent periodic scan retries it after failure or restart. It persists a sidecar identity and connect deadline before calling `ensure`, parks an accepted generation while waiting for its WebSocket, and wakes immediately on an exact-generation connect event. A disconnect starts durable reconnect grace. The runtime periodically repairs an allocated generation that has neither a live connection nor a durable schedule, without replacing a concurrent wake or reconciliation lease. If the deadline expires, the default behavior atomically fails the deployment's active runs, deactivates their principals, advances the fence, and releases the uncertain worker. Trigger enqueue transactions lock and revalidate the allocation, reread the authoritative Git lifecycle under that fence, and revalidate the deployment's SQL run state, so they cannot enqueue after either representation becomes terminal. An operator may explicitly enable automatic recovery; only then does the reconciler provision the next generation under the same allocation.
...
Before sending an allocated deploy, the Hub commits an `initialization_lease_id` on the allocation and clears the anchor's previous public key in one transaction under the current accepted generation and unexpired reconciliation lease. Restoration and preparation happen before this reservation. The router rechecks cancellation and the exact socket after the reservation commits. A crash between reservation and send can conservatively require cleanup even if no frame reached the worker. A confirmed reservation returns the previous public key to the current attempt without storing a second key in the database. A proven unsent failure restores that key and clears its own initialization marker atomically, even after cancellation or loss of its reconciliation lease, provided the exact marker, allocation status, anchor identity, and accepted generation still match. If reservation confirmation is lost, the caller has no captured key and leaves the marker for conservative cleanup. Cancellation, disconnects, retries, and lease changes otherwise preserve the marker; a stale clear cannot remove a newer attempt or affect another generation.

An allocated supervisor's deploy acknowledgement does not publish its public key immediately. The Hub publishes the key and credential references and clears the exact initialization marker in one allocation-locked transaction, rechecking lease expiry before committing the mutations. A later reconciliation with an outstanding marker fences and releases the generation, or replaces it when recovery is explicitly enabled, before checking connectivity or restoring history. That cleanup requires the exact observed marker to remain present when the generation advances. A concurrent proven-unsent rollback instead requeues ordinary initialization checks, including when the first attempt had no previous public key; absence of the marker alone does not publish readiness. Readiness and dispatch require the marker to be absent, while bootstrap Git uploads remain permitted. Release and replacement consume the marker atomically with the generation advance. A publication error whose transaction actually committed must not release the completed worker: cleanup revalidates the marker and key after acquiring the allocation lock, and completed initialization re-enters the normal callback to finish dispatch requeue and readiness. An active allocated supervisor without a completed initialization key remains uncertain and must be fenced and released or replaced, never redeployed in place.

Mail sent before initialization publishes its signing key is deferred under the allocation generation and reconciliation attempt. Cancellation leaves that mail pending because the key transaction may already have committed. A confirmed unsent rollback releases queued mail with the restored key, or fails it if no previous key existed. A new reconciliation resolves the previous attempt's marker and key before waiting for a connection. A completed or rolled-back initialization releases its queued mail; an outstanding marker leaves mail pending until fencing confirms failure, because the prior caller may still complete a proven-unsent rollback. A lost rollback response is resolved from the restored durable key on a later claim. A durable generation advance also clears an unfinished sender attempt, including when the cleanup transaction's response was lost. Late callbacks can settle only their own attempt. Deferred delivery carries the confirmed signing key so a newer attempt cannot capture the queued message or change its verification key. The Hub completes asynchronous delivery preparation before sending the sender key and mail consecutively on the same socket. The existing deferred-mail TTL remains the fallback if recovery cannot resolve the outcome.
...
- When a generation becomes ready, pending and acknowledged dispatches are requeued. Abandoned attempts remain stopped. Mail claim checks and signal ids make replay idempotent across a worker loss at any point in delivery.
- A terminal target run closes dispatches to that run after accepted ids in its log are settled. The anchor's terminal log closes dispatches to a still-live child only after the anchor's database row is terminal or its allocation is closed, no pack projection is pending, and the pinned Git tree is still current. A failed anchor status projection can otherwise leave the allocation able to accept a later child pack. This ordering also prevents the terminal event from racing the firing mail's immediately-following claim-check receipt.
- Forced stop and unrecoverable allocation failure abandon outstanding attempts without asserting rejection. A bounded independent recovery loop scans unresolved dispatches regardless of the database run status and can resolve abandoned attempts from later Git evidence. Mail used to resume an input gate is accepted through a `SignalReceived` event, just as an explicit signal is; the dispatch id remains unique across both kinds within the anchor.

Run statuses have a matching obligation in `workflow_pending_projection`. Pack ingestion inserts a row before the receive can advance Git and deletes it after every newly terminal run in the pack reaches a final decision, or when the receive is rejected before Git changes. A row that survives a failed projection or a failed receive is reconciled by the lifecycle sweep once it is older than a short grace: the Hub reads the deployment's accepted history at one pinned tip, records each run's terminal outcome (creating child rows it never recorded), confirms the tip under the allocation lock, and deletes only rows whose receive has finished in this process or belonged to an earlier one. A receive writes the ref last and nothing removes refs, so a repository without a ref never advanced and its row clears, while a missing repository under a receive's row fails reconciliation instead of clearing it; only rows the migration backfilled for deployments that may never have pushed clear on a missing ref. A confirmed stop records `cancelled` only once the Hub holds the ref tips the stopped worker reported and no row remains, so it cannot overwrite an outcome the worker committed or one the database has not seen yet. A stop the worker cannot confirm ends in allocation release instead, and `cancelled` is then recorded once no row remains, over whatever outcome the worker committed but never delivered. Unrecoverable allocation failure follows the same rule: while a row remains it records `infrastructure_failed_at` on the anchor instead of failing live runs, and the lifecycle sweep fails the runs still live, at that time, once reconciliation has cleared every row.
