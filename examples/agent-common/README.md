...
- `resolveProvider({ env, providerOverride, exampleName })` returns a
  `ProviderConfig` derived from `ANTHROPIC_API_KEY` (with a default
  model and base URL), or returns a `{ ok: false, help }` object whose
  `help` string is a friendly multi-line message the caller writes to
  stderr before exiting non-zero. Tests bypass env resolution entirely
  by passing `providerOverride` directly.
