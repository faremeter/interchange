...
const logger = getLogger([
  "gemini-discover",
  "function-calling-thinking-streaming",
]);

const NAME = "function-calling-thinking-streaming";
...
export const capability: Capability = {
  ...
  build: async ({ apiKey, scriptVersion }) => {
    ...
    if (sigCount === 0) {
      throw new Error(
        `${NAME} turn-1: streamed response contained no thoughtSignature; thinking did not engage`,
      );
    }
    logger.info`turn-1 streamed ${String(sigCount)} thoughtSignature(s) and functionCall name=${modelTurn.functionCallName}`;
