...
// Publish a directory of tool-package tarballs into a tenant's
...
//
// Auth pattern: the hub's REST endpoints are session-gated; this script
// reuses the same sign-in flow that `bin/seed.ts` uses, since there is
// no pre-existing bearer-token convention for human-facing assets. The
// admin credentials default to the seed's `alice@example.com` /
// `password123`; in dev orchestration `bin/dev.ts` and the in-process
// publish path below share these defaults. Operators running this
// against a non-dev hub set `HUB_ADMIN_EMAIL` and `HUB_ADMIN_PASSWORD`.
//
// The script is idempotent: re-running uploads every tarball under
...
// function" deep in the auth flow.
...
async function runCLI(): Promise<void> {
  ...
  if (values.help) {
    console.log(
      "Usage: bun bin/publish-tool-packages.ts [--registry <name>] [--from <dir>] [--tenant <slug>] [--tenant-name <name>]",
    );
    console.log("");
